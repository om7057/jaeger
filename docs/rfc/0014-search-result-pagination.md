...
- ✅ The in-memory backend is the first reader to declare `Paginated = true`: `FindTraceIDs` orders the matching traces by §3.3's key, `FindSpans` by RFC 0016 §6's, and each pages with the last result's key as the cursor inside the §3.1 token. Delivered with RFC 0016 M3 in [#9611](https://github.com/jaegertracing/jaeger/pull/9611).
