...
- ✅ Pagination of the memory backend's `FindSpans` (§6): the matching spans are ordered by `(startTime desc, traceID asc, spanID asc)`, the page is bounded by `page_size`, and the last span's key is the cursor inside [RFC 0014](0014-search-result-pagination.md)'s token. The backend declares `SearchCapabilities.Paginated = true`, which is one capability for both searches, so its `FindTraceIDs` pages the same way by RFC 0014 §3.3's key.
